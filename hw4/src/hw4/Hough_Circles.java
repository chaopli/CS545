...
public class Hough_Circles implements PlugInFilter {
    ...
    public void run(ImageProcessor ip) {
    	...
    	edge_detector.start(process_image);
    	process_image = edge_detector.get_result(); 
    	process_image.show();
        ...
        result.setProcessor(circlesip.duplicate());
        getCenterPoints(maxCircles);
        drawCircles(circlespixels);
        result.setTitle(maxCircles+" Circles Found");
        result.show();
	...
	public void drawCircles(byte[] circlespixels) {
		...
		for( int y = offy; y < offy+height; y++) {
			for(int x = offx; x < offx+width; x++) {
				int v = imageValues[x+offset*y];
				if (v != 0)
					v = 255;
				else v = 0;
				result.getProcessor().putPixel(x, y, v);
			}
		}
		...
		for (int i = 0; i < result.getHeight(); i++)
		{
			...
			{
				result.getProcessor().putPixel(j, i, 255 - result.getProcessor().getPixel(j, i));
			}
		}
		if(centerPoint == null) {
			getCenterPoints(maxCircles);
...
//		
		for(int l = 0; l < maxCircles; l++) {
			...
			result.getProcessor().drawOval(i, j, 3, 3);
		}
	}
...
class Canny_Edge_Detector implements PlugIn {
	...
	public void start(ImagePlus ij) {
		sourceImage = ij.duplicate();
